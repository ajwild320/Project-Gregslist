...
def test_update_listing(test_app):
    ...
    #Signs up, adds listing, checks to see if it exists
        ...
        response = test_app.get(f"/items/{item.item_id}", follow_redirects = True)
        assert response.status_code == 200
        data = response.data.decode('utf-8')
        ...
        #making sure other users cant delete other's posts
        response = test_app.get(f"/items/delete/{item.item_id}", follow_redirects=True)
        assert response.status_code == 200
        data = response.data.decode('utf-8')
        assert "My Account" in data
